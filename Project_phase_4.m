...
% FPR_1_p=FPR_1(1:153,2:2);
% 
% filename = 'FOPR_case_2';
% FOPR_1 = xlsread(filename);
% FOPR_1_t=FOPR_1(1:153,1:1);
% FOPR_1_p=FOPR_1(1:153,2:2);
% 
% filename = 'FGPR_case_2';
% FGPR_1 = xlsread(filename);
% FGPR_1_t=FGPR_1(1:153,1:1);
% FGPR_1_p=FGPR_1(1:153,2:2);
...
% percent_variation=0.3;
% info.kx_res_orig=info.kx_res_orig+percent_variation*info.kx*rand(info.Ny,info.Nx)-0.5*percent_variation*info.kx;
% info.ky_res_orig=1.5*(info.ky_res_orig+percent_variation*info.ky*rand(info.Ny,info.Nx)-0.5*percent_variation*info.ky);

% % channal
% k_chan=2000;
% info.kx_res_orig(12,:)=k_chan;
% info.ky_res_orig(12,:)=k_chan*120/80;
...
% P_matrix_n1_k =P_matrix(info.P_res_n1,info.S_res_n1)+dP_matrix_n;
P_matrix_n1_k =P_matrix(info.P_res_n1,info.S_res_n1);
...
D_matrix_new_out=D_matrix_new(info,P_matrix_n,P_matrix_n1_k);
% make T matrix
...
% % make R matrix
% if P_matrix_n1_k==P_matrix_n
%     R_matrix=Q_matrix_out;
% else
% % R_matrix=(T_matrix_out*P_matrix_n1_k-D_matrix_out*(P_matrix_n1_k-P_matrix_n)+Q_matrix_out);
% D_matrix_comp=-D_matrix_out*(P_matrix_n1_k-P_matrix_n);
R_matrix=(T_matrix_out*P_matrix_n1_k+D_matrix_new_out+Q_matrix_out);
% end

% set everything to 0???????????????????????
% get do the volume normalized??????????????????????????


% R_matrix=(T_matrix_out*P_matrix_n1_k+D_matrix_new_out);
% end
...
delta_p_max=max(max(abs((P_matrix_n1_k_2D_P-P_matrix_n_P)/mean(P_matrix_n_P))));


CFL_max_vec=[CFL_max_vec,CFL_max];
iter_newton_vec=[iter_newton_vec,iter_newton];
delta_t_vec=[delta_t_vec,info.delta_t];
