...
myFolder = 'C:\Users\nden\Desktop\old stuffs\Stanford\ENERGY223\Project\project4_Nutchapol_Dendumrongsup_IMPES\debug_plots';
...
% Fluid properties
info.P_bub=4500;%3400;%*6894.76; % psi to Pa
...
%     while (delta_p_max > 10 && t_total > 40) || loop
        ...
        [row,col]=size(L);
        I=eye(row,col);
        M1=pinv_L*(exp_Lh-I)+delta_t_n*exp_Lh*(I-pinv_L*L);
        M2=pinv_L*pinv_L*(exp_Lh-(I+L*delta_t_n))+(1/2)*delta_t_n^2*exp_Lh*(I-pinv_L*L);
%         % filter
%         M1(M1<1e-4)=0;
%         M2(M2<1e-4)=0;
       
        an=exp_Lh*Pressure_matrix_n+M1*N_matrix_IMPES(info,Pressure_matrix_n,Saturation_matrix_n,delta_t_n,control);
        Pressure_matrix_n1=an+delta_t_n^-1*M2*(N_matrix_IMPES(info,an,Saturation_matrix_n,delta_t_n,control)-N_matrix_IMPES(info,Pressure_matrix_n,Saturation_matrix_n,delta_t_n,control));
        ...
        % update
        Pressure_matrix_n=Pressure_matrix_n1;
        Saturation_matrix_n=Saturation_matrix_n1;
        end
