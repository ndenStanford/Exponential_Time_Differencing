...
while (t_total<info.t_runtime) %% && (p_well>info.p_switch)
    while delta_p_max > 50
        ...
        [row,col]=size(L);
        I=eye(row,col);
        M1=pinv_L*(expm(L*delta_t_n)-I)+delta_t_n*expm(L*delta_t_n)*(I-pinv_L*L);
        M2=pinv_L*pinv_L*(expm(L*delta_t_n)-(I+L*delta_t_n))+(1/2)*delta_t_n^2*expm(L*delta_t_n)*(I-pinv_L*L);
        an=expm(L*delta_t_n)*Pressure_matrix_n+M1*N_matrix_IMPES(info,Pressure_matrix_n,Saturation_matrix_n,delta_t_n,control);
